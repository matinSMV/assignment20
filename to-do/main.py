...
class TODO(QMainWindow):
    ...
    def readFromDatabase(self):
        ...
        for i in range(len(results)):
            ...
            if results[i][2] == '0':
                ...
                self.new_btn_do.clicked.connect(
                    partial(self.do, results[i], self.new_btn_info, self.new_lbl_title, self.new_btn_do, self.new_check,
                            self.new_btn_delete))
                ...
                self.new_btn_delete.clicked.connect(
                    partial(self.remove, results[i], self.new_btn_info, self.new_lbl_title, self.new_btn_do,
                            self.new_check, self.new_btn_delete))
                ...
                self.new_btn_undo.clicked.connect(
                    partial(self.undo, results[i], self.new_btn_info, self.new_lbl_title, self.new_btn_undo,
                            self.new_check, self.new_btn_delete))
                ...
                self.new_btn_delete.clicked.connect(
                    partial(self.remove, results[i], self.new_btn_info, self.new_lbl_title, self.new_btn_undo,
                            self.new_check, self.new_btn_delete))
    ...
    def do(self, i, btn_info, lbl_title, btn_do, check, btn_delete):
        if check.isChecked() == True:
            ...
            btn_delete.deleteLater()
            msg = QMessageBox()
            msg.setText("change saved!")
            msg.exec()
        else:
            msg = QMessageBox()
            msg.setText("change does not saved!")
            msg.exec()
